...
| `city` | string | yes | — | Czech city name with diacritics, Title Case. |
| `district` | string \| null | no | `null` | Optional district. |
...
| `job_started_at` | string \| null | no | `null` | Set when entering `running`. |
| `job_completed_at` | string \| null | no | `null` | Set when entering `completed` or `failed`. |

11 fields total. 7 required, 4 optional. No nested objects.

## 3. Validation rules

### Required
All 7 required fields must be present and non-empty. `additionalProperties: false`.
...
- `source_job_id`: regex `^[a-z0-9]+-\d{8}T\d{6}Z-[0-9a-f]{10}$`.
...
### Examples
```
firmycz-20260405T143022Z-9a3f2b1c4e
zivefirmycz-20260405T090000Z-1d2e3f4a5b
firmycz-20260412T143022Z-9a3f2b1c4e   (same scope, different day = different id)
```
...
{
  "schema_version": "1.0",
  "source_job_id": "zivefirmycz-20260405T090000Z-1d2e3f4a5b",
  "portal": "zivefirmy.cz",
  "segment": "elektrikar-rev",
  "city": "Hradec Kralove",
  "district": "Novy Hradec Kralove",
  "max_results": 500,
...
```
Borderline because `max_results = 500` is at the maximum (soft warning on Serper/ToS risk), `segment` contains a hyphen (valid but edge), `district` contains spaces.
