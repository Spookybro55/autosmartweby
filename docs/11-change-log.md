...
- **Secrets inventory — 10-field contract:** `key` / `provider` (Script Properties / GCP Secret Manager / external vault) / `rotation_policy` (manual / 90d / 180d / on-incident) / `redaction_mask_pattern` / `used_in_step[]` / `access_pattern` (read-at-start / read-per-call / read-on-rotation) / `timing_insensitive_compare` (bool) / `pii_class` (none / low / high) / `last_rotated_at` / `owner`. **5 VERIFIED** (ASW_SPREADSHEET_ID — not a secret but bound to env; PREVIEW_WEBHOOK_SECRET — VERIFIED timing-insensitive compare in doPost; SERPER_API_KEY — VERIFIED used by A-04 firmy.cz scraper; EMAIL_MAILBOX_ACCOUNT const — VERIFIED but should migrate to Script Property; ASW_ENV — VERIFIED env-selector not strictly secret) + **2 PROPOSED** (SENDGRID_API_KEY / MAILGUN_API_KEY conditional on `EMAIL_PROVIDER` selection per C-06).
- **Per-stage limits for 9 pipeline stages:** A-04 firmy.cz scraper (PROPOSED LIMIT_SCRAPE_BATCH_SIZE=200, LIMIT_SCRAPE_MAX_RUNTIME_MS=270000, LIMIT_SCRAPE_MAX_RETRIES=3), A-05 dedupe (VERIFIED BATCH_SIZE=100 → PROPOSED LIMIT_DEDUPE_BATCH_SIZE=100), A-06 auto web check (PROPOSED LIMIT_WEBCHECK_BATCH_SIZE=80 — soucasny hardcode, LIMIT_WEBCHECK_MAX_SERPER_CALLS_PER_RUN=80), A-07 auto qualify (PROPOSED LIMIT_QUALIFY_BATCH_SIZE=100), A-08 preview queue (PROPOSED LIMIT_PREVIEW_BATCH_SIZE=50), B-04 preview render endpoint (PROPOSED LIMIT_PREVIEW_RENDER_MAX_CONCURRENT=5), C-05 outbound queue worker (PROPOSED LIMIT_OUTBOUND_WORKER_BATCH_SIZE=50, LIMIT_OUTBOUND_WORKER_MAX_RUNTIME_MS=240000), C-06 provider send (PROPOSED LIMIT_PROVIDER_SEND_PER_MIN=60, LIMIT_PROVIDER_SEND_PER_DAY=2000 — Gmail API soft limit), C-08 follow-up engine (PROPOSED LIMIT_FOLLOWUP_BATCH_SIZE=100, LIMIT_FOLLOWUP_MAX_SEQUENCES_PER_DAY=500). Invariant: limits jsou **hard caps per stage** — pri prekroceni stage safe-stopne s `limit_exceeded` event, nesmi se retryovat proti limitu.
