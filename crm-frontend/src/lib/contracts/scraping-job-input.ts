...
export interface ScrapingJobInput {
  ...
  /**
   * Deterministic unique job id.
   * Format: `{portal_slug}-{yyyymmddThhmmssZ}-{hash10}`
   * See contract doc section 6 for generation rules.
   */
  ...
  /** Czech city name, Title Case, UTF-8 with diacritics. Example: "Plzen". */
  city: string;

  /** Optional district or borough. `null` for whole-city job. */
  district: string | null;

  /** Max results to scrape. 1..500, default 50. */
  max_results: number;

  /** ISO 8601 UTC, always ends with `Z`. Example: "2026-04-05T14:30:22Z". */
  job_created_at: string;

  /** Lifecycle status. Starts at "created". */
  job_status: ScrapingJobStatus;

  /** ISO 8601 UTC. `null` until job transitions to "running". */
  job_started_at: string | null;

  /** ISO 8601 UTC. `null` until job reaches "completed" or "failed". */
